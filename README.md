...
- **Cash-Limit:** vor jedem Entry wird `TotalCashValue` (Basiswährung EUR)
  ...
  jeweils genug Cash da ist — es gibt keine Margin-Nutzung. Die Abfrage
  läuft über `reqAccountSummary`, das alle Werte bereits in Kontowährung
  liefert und als Währung deren echtes Kürzel (z. B. `EUR`) meldet — nicht
  das Pseudo-Kürzel `BASE`, das es nur bei `reqAccountUpdates` gibt.
  Passt keine Zeile zu `ACCOUNT_CURRENCY`, wird der Entry abgelehnt statt
  einen Fremdwährungsbetrag als Kontowährung zu verrechnen.
